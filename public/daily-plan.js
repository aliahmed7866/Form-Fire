...
async function todayPage(d) {
  ...
  const shopping=plans.filter(a=>a.kind==='meal'&&a.snapshot.shopping_list).map(a=>`<details><summary>${esc(a.title)} · shopping list</summary><p class="micro">Follow the quantities and serving notes agreed with Alex.</p><div class="pre">${esc(a.snapshot.shopping_list)}</div></details>`).join('');
  return `<div class="daily-welcome">${header}${artwork('train','today-art')}</div>`+controls+requestsNote+sections+shopping+`<section class="weekly-history"><div class="section-top"><div><div class="eyebrow">${monday} to ${sunday}</div><h2>A few steps <span class="serif">worth noticing.</span></h2></div></div><p class="micro">Only what you choose to log. A blank day isn’t a judgement.</p>${activityHistory(d,entries)}</section>`;
}
