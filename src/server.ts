...
export function createApp(options:{dataDir?:string,origin?:string,google?:GoogleConfig,googleFetch?:typeof fetch}={}) {
  ...
  const handle=async(req:IncomingMessage,res:ServerResponse)=>{
    ...
    try {
      ...
      if(session&&method!=='GET')check(req.headers['x-csrf-token']===session.csrf,'Refresh this page and try again (session check).',403);
      ...
      if(p==='/api/auth/logout'&&method==='POST') { if(session)run('DELETE FROM sessions WHERE id=?',session.id);res.setHeader('Set-Cookie','ff_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0');return send(200,{ok:true}); }
      ...
      if((p==='/api/dashboard'||p==='/api/export')&&method==='GET') {signed();const result={user:safeUser(user),requests:all('SELECT r.*,s.title service_title FROM requests r JOIN services s ON s.id=r.service_id WHERE user_id=? ORDER BY r.created_at DESC',user.id).map(decorateRequest),assignments:all('SELECT * FROM assignments WHERE user_id=? ORDER BY created_at DESC,rowid DESC',user.id).map(a=>({...a,snapshot:parse(a.snapshot)})),checkins:all('SELECT * FROM checkins WHERE user_id=? ORDER BY week DESC',user.id),invoices:all('SELECT * FROM invoices WHERE user_id=? ORDER BY created_at DESC',user.id).map(decorateInvoice),account_requests:all('SELECT * FROM account_requests WHERE user_id=?',user.id),...(p==='/api/export'?{activity:clientActivity(db,user.id)}:{})};if(p==='/api/export')res.setHeader('Content-Disposition','attachment; filename="form-fire-export.json"');return send(200,result);}
