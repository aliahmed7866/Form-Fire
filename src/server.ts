...
export function createApp(options:{dataDir?:string,origin?:string}={}) {
  ...
  const server=createServer(async(req,res)=>{
    ...
    try {
      ...
      if((p==='/api/dashboard'||p==='/api/export')&&method==='GET') {signed();const result={user:safeUser(user),requests:all('SELECT r.*,s.title service_title FROM requests r JOIN services s ON s.id=r.service_id WHERE user_id=? ORDER BY r.created_at DESC',user.id).map(decorateRequest),assignments:all('SELECT * FROM assignments WHERE user_id=? ORDER BY created_at DESC,rowid DESC',user.id).map(a=>({...a,snapshot:parse(a.snapshot)})),checkins:all('SELECT * FROM checkins WHERE user_id=? ORDER BY week DESC',user.id),invoices:all('SELECT * FROM invoices WHERE user_id=? ORDER BY created_at DESC',user.id).map(decorateInvoice),account_requests:all('SELECT * FROM account_requests WHERE user_id=?',user.id)};if(p==='/api/export')res.setHeader('Content-Disposition','attachment; filename="form-fire-export.json"');return send(200,result);}
